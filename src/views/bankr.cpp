...
void drawList()
{
	...
	for (int row = 0; row < LIST_ROWS; row++) {
		...
		ui::usd(a.mcap, text, sizeof(text));
...
void drawDetail()
{
	...
	char values[4][24];
	ui::usd(a.mcap, values[0], sizeof(values[0]));
	ui::usd(a.volume, values[1], sizeof(values[1]));
...
void drawScore()
{
	...
	ui::clearBody();
	if (page == 0) {
		coral::drawRead(score, a.symbol[0] == '\0' ? a.name : a.symbol);
	} else {
		coral::drawWhy(score);
	}
}
